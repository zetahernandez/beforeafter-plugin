var scompimg = Class.create({
  ...
  build: function() {
    //init images
    this.fimg = new Element('img', {
      src: this.o.fimg,
      class: 'scompimg-img fimg'
    });
    this.simg = new Element('img', {
      src: this.o.simg,
      class: 'scompimg-img simg'
    });
    _this = this;
    this.fimg.on('load', function(evt) {
      _this.o.width = evt.target.width;
      _this.o.height = evt.target.height;
      $(_this.div, _this.fimg, _this.simg, _this.fdiv, _this.sdiv, _this.contents).invoke('setStyle', {
        width: _this.o.width + 'px',
        height: _this.o.height + 'px'
      });
      _this.reSize((_this.o.width / 2) - 1, true);
    });
    ...
    this.contents = new Element('div', {
      class: 'scompimg-contets'
    }); //div
    this.fdiv = new Element('div', {
      class: 'scompimg-imgc fimg'
    }).insert({
      top: this.fimg
    }); //first
    this.sdiv = new Element('div', {
      class: 'scompimg-imgc simg'
    }).insert({
      top: this.simg
    }); //second

    this.links = new Element('div', {
      class: 'scompimg-links clearfix'
    }).insert({
      top: new Element('a', {
        class: 'scompimg-before',
        href: '#'
      }).update(this.o.btxt),
      bottom: new Element('a', {
        class: 'scompimg-after',
        href: '#'
      }).update(this.o.atxt)
    });
    ...
    }).addClassName('sod-scompimg');


    this.fdiv.insert({
      after: this.o.boxTemp.evaluate()
    });
  ...
  changePos: function(e) {
    var t = e.pageX - this.div.viewportOffset().left,
      noEffect = (!Object.isUndefined(this.mv)) ? this.mv.started ? true : false : false;
  ...
  reSize: function(pos, noEffect) {
    if (!noEffect)
      return new Effect.Tween(null, this.fdiv.getWidth(), pos, {
